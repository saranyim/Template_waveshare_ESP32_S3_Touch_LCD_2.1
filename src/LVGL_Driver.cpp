...
{
  lv_init();
  #define BUFFER_PIXELS (ESP_PANEL_LCD_WIDTH * ESP_PANEL_LCD_HEIGHT)

  buf1 = (lv_color_t *)ps_malloc(sizeof(lv_color_t) * BUFFER_PIXELS);
  buf2 = (lv_color_t *)ps_malloc(sizeof(lv_color_t) * BUFFER_PIXELS);
  if (!buf1 || !buf2) 
  {

    ESP_LOGE("LVGL", "Failed to allocate LVGL draw buffer!");
    // ไม่ควร init LVGL ถ้าไม่มี buffer
  }
  // esp_lcd_rgb_panel_get_frame_buffer(panel_handle, 2, &buf1, &buf2);                                          
  lv_disp_draw_buf_init( &draw_buf, buf1, NULL, ESP_PANEL_LCD_WIDTH * ESP_PANEL_LCD_HEIGHT);           
