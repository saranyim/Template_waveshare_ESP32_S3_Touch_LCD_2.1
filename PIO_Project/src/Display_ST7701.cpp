...
void ST7701_Init()
{
  ...
  //  RGB
  esp_lcd_rgb_panel_config_t * rgb_config = (esp_lcd_rgb_panel_config_t *)heap_caps_calloc(1, sizeof(esp_lcd_rgb_panel_config_t), MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);
    ...
    rgb_config->clk_src = LCD_CLK_SRC_PLL160M,
      ...
      rgb_config->timings.flags.hsync_idle_low = 0;  /*!< The hsync signal is low in IDLE state */
      rgb_config->timings.flags.vsync_idle_low = 0;  /*!< The vsync signal is low in IDLE state */
      rgb_config->timings.flags.de_idle_high = 0;    /*!< The de signal is high in IDLE state */
      rgb_config->timings.flags.pclk_active_neg = false;
      rgb_config->timings.flags.pclk_idle_high = 0;  /*!< The PCLK stays at high level in IDLE phase */

      rgb_config->data_width = ESP_PANEL_LCD_RGB_DATA_WIDTH;
      // rgb_config->bits_per_pixel = ESP_PANEL_LCD_RGB_PIXEL_BITS;
      // rgb_config->num_fbs = ESP_PANEL_LCD_RGB_FRAME_BUF_NUM;
      // rgb_config->bounce_buffer_size_px = 10 * ESP_PANEL_LCD_HEIGHT;
      ...
      rgb_config->flags.relax_on_idle = 0;
  ...
  // uint16_t *test_fb = (uint16_t *)ps_malloc(sizeof(uint16_t) * ESP_PANEL_LCD_WIDTH * ESP_PANEL_LCD_HEIGHT);
  // for (int i = 0; i < ESP_PANEL_LCD_WIDTH * ESP_PANEL_LCD_HEIGHT; i++) test_fb[i] = 0xF800;  // สีแดง
  // esp_lcd_panel_draw_bitmap(panel_handle, 0, 0, ESP_PANEL_LCD_WIDTH, ESP_PANEL_LCD_HEIGHT, test_fb);
  }
